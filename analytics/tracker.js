'use strict';
/**
 * Request analytics within the ADR-021 privacy bounds. Drop-in replacement for the pre-ADR
 * openvibe-shared AnalyticsTracker (same constructor, tables and getStats/getOverview/getBotAnalysis
 * shapes), minus the personal data:
 *
 ...
 *
 *   opt-out      a request with `Sec-GPC: 1` or `DNT: 1` is not recorded at all (privacy.optedOut):
 *                no row, no visitor hash, no session id, no rate counter.
 *
 * Retention of raw events (30 days) is ./retention.js; this tracker writes, rolls up and (with
 * opts.retention) schedules the prune. The row shape is analytics/event.v1 (./event.js).
 */
...
class AnalyticsTracker {
     ...
     *        (added to privacy.DEFAULT_PARAM_PREFIXES), e.g. Network's ['avatar', 'anon', 'projects']
     * @param {Array<[RegExp, string]>} [opts.pathRules]  rewrites applied to a raw path before it is
     *        templated, for parameters the segment rules cannot see (privacy.normalisePath)
     * @param {false|object} [opts.retention]  { days = 30, intervalMs = 24 h, initialDelayMs = 5 min }:
     *        the raw-event prune on a timer (retention.schedulePrune; the job is this.pruneJob). On by
     *        default (ADR-021: raw events at most 30 days); pass false only when the service schedules
     *        retention.pruneRawEvents itself.
    ...
    constructor(db, service, opts = {}) {
        ...
        if (opts.timers !== false) {
            ...
            if (opts.retention !== false) {
                const r = opts.retention && typeof opts.retention === 'object' ? opts.retention : {};
                this.pruneJob = retention.schedulePrune(db, { label: `Analytics:${service}`, now: this._now, ...r });
            }
        }
    }
    ...
    destroy() {
        ...
        this._timers = [];
        if (this.pruneJob) { this.pruneJob.stop(); this.pruneJob = null; }
        this._rates.clear();
        this._sessions.clear();
    }
}
