 ...
 *   schedulePrune(db, opts)        the nightly prune on unref'd timers (first run 5 min after boot,
 *                                  then every 24 h); returns { run, stop }.
 *
 * scrubEvents / scrubRollups / inspect take the service's path options ({ paramPrefixes, pathRules },
 * see privacy.normalisePath) so legacy rows are templated with the same rules as new ones.
 *
 * Used by the tracker (opts.retention), a service's own job runner, and ./prune-cli.js.
 */
...
function registerFunctions(db, { paramPrefixes, pathRules } = {}) {
    const opts = privacy.pathOptions({ paramPrefixes, pathRules });
...
async function scrubEvents(db, { batchSize = DEFAULT_BATCH, paramPrefixes, pathRules } = {}) {
    ensureSchema(db);
    registerFunctions(db, { paramPrefixes, pathRules });
...
function scrubRollups(db, { paramPrefixes, pathRules } = {}) {
    const opts = privacy.pathOptions({ paramPrefixes, pathRules });
...
function inspect(db, { days = MAX_DAYS, now = Date.now, paramPrefixes, pathRules } = {}) {
    const cutoff = cutoffFor(days, now());
...
/**
 * Run pruneRawEvents(db, { days }) on timers: first after `initialDelayMs` (default 5 min), then every
 * `intervalMs` (default 24 h). Timers are unref'd, so they never hold the process open. An overlapping
 * run is skipped (resolves null). `label` names the service in log lines.
 * Returns { run, stop }: run() prunes now (resolves the prune result, or null on error/overlap).
 */
function schedulePrune(db, {
    days = MAX_DAYS, batchSize = DEFAULT_BATCH, initialDelayMs = 5 * 60 * 1000, intervalMs = 24 * 60 * 60 * 1000,
    label = 'Analytics', log = console.log, logError = console.error, now = Date.now,
} = {}) {
    checkDays(days);
    let running = false;
    const run = async () => {
        ...
        running = true;
        try {
            const out = await pruneRawEvents(db, { days, batchSize, now });
            if (out.deleted) log(`[${label}] pruned ${out.deleted} raw events older than ${out.cutoff}`);
            return out;
        } catch (err) {
            logError(`[${label}] analytics-prune error:`, err.message);
            return null;
        } finally {
            running = false;
        }
